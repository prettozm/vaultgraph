# Vault Graph — prompt d'installation (copier-coller intégral)

> Remplace `<REPO_URL>` par l'URL de ton dépôt GitHub public, puis colle **tout ce fichier** à Claude.

---

Dépôt cible : `<REPO_URL>`

Installe et initialise Vault Graph 0.1 sur ce dépôt, conformément au protocole ci-dessous.
Ce message est auto-suffisant : tout le contrat est embarqué, tu n'as rien d'autre à télécharger.

## Ce que tu dois faire

1. Accède au dépôt `<REPO_URL>` (dépôt public ; clone-le ou lis-le via l'API GitHub).
2. Détermine sa structure : branche par défaut (ne suppose pas `main`), dossiers documentaires, formats présents.
3. Crée `.vault-graph/` à la racine, avec **exactement** l'arborescence et le contenu des fichiers embarqués plus bas.
4. Adapte `.vault-graph/config.yaml` à ce dépôt : ajuste `scan.include` / `scan.exclude` aux formats et
   dossiers réellement présents (exclus toujours `.git/**`, `.vault-graph/**`, `node_modules/**` et les artefacts générés).
   Ne touche pas à `write_scope`.
5. Propose le `schema.yaml` **minimal nécessaire** : pars des valeurs par défaut embarquées, retire ce qui
   n'a aucun sens ici, ajoute uniquement les types ou relations que le contenu impose. Justifie chaque écart dans `reports/build.md`.
6. Scanne les sources selon `config.yaml` : calcule le `sha256` de chaque fichier retenu.
7. Produis le premier graphe (mode INIT) en suivant `.vault-graph/INSTRUCTIONS.md` : nœuds, arêtes, provenance,
   réconciliation, orphelins expliqués. Écris `graph/nodes.jsonl`, `graph/edges.jsonl`, puis recalcule `graph/graph.json`.
8. Produis les rapports : `reports/build.md`, `reports/candidates.md`, `reports/unresolved.md`.
9. Valide : renseigne `manifest.json` (`generated_at` = maintenant en UTC ISO-8601, `source.commit` = `git rev-parse HEAD`,
   `source.branch`, `generator.name` = ton nom), reporte les mêmes valeurs dans `graph/graph.json`,
   puis exécute la **checklist de validation §13** de `INSTRUCTIONS.md` et publie son résultat dans `reports/build.md`.
10. **N'écris et ne modifie aucun fichier hors de `.vault-graph/`.** Les documents source restent intacts.

Termine en affichant le contenu de `.vault-graph/reports/build.md`.

Une fois l'installation faite, la mise à jour ne nécessite plus ce prompt :
« Lis `.vault-graph/INSTRUCTIONS.md` et mets à jour le Vault Graph. »

---

# Fichiers à matérialiser
...
├── graph/
│   ├── graph.json
│   ├── nodes.jsonl        (vide au départ)
│   └── edges.jsonl        (vide au départ)
├── state/
│   └── state.json
└── reports/
    ├── build.md
...
Squelette (tu renseignes `generated_at`, `source.commit`, `source.branch`, `generator.name` à l'étape 9) :
...
## `.vault-graph/config.yaml`

Valeurs par défaut, à adapter à l'étape 4 :
...
## `.vault-graph/schema.yaml`

Valeurs par défaut, à réduire ou étendre à l'étape 5 :
...
À écrire **verbatim** dans le dépôt : c'est ce fichier qui rendra les mises à jour futures possibles sans ce prompt.
...
## `.vault-graph/graph/graph.json`

Squelette (recalculé à l'étape 7) :
...
## `.vault-graph/graph/nodes.jsonl` et `.vault-graph/graph/edges.jsonl`

Deux fichiers **vides** (0 octet) à la création. Ensuite : un objet JSON complet par ligne, sans virgule de séparation.

## `.vault-graph/state/state.json`

Squelette (rempli à l'étape 6) :
...
## `.vault-graph/reports/candidates.md`
...
## `.vault-graph/reports/unresolved.md`
...
Rappel final : écriture limitée à `.vault-graph/`, aucune provenance inventée, aucune fusion silencieuse,
aucune hypothèse promue en fait, checklist §13 exécutée avant d'annoncer que c'est terminé.
