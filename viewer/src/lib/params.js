...
/** Build the shareable/bookmarkable URL for a given target. */
export function buildAppUrl(baseUrl, target) {
  const u = new URL(baseUrl);
  u.search = '';
  if (target?.kind === 'manifest') u.searchParams.set('manifest', target.value);
  else if (target?.kind === 'repo') u.searchParams.set('repo', target.value);
  return u.toString();
}
